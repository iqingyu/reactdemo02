...
class FetchUtil {
      ...
      .then(
        function(response) {
          ...
          if (response.ok) {
            ...
            var error = new Error(this.getHttpErrorMsg(response.status));
  ...
  doHttp(url, option, method, body) {
    option = option || this.getDefaultOption();

    if (option.start) {
      option.start();
    }

    let doOption = Object.assign({}, option.fetchOption, {
      method: "" + method
    });
    ...
    this.thenCatchFinally(promise, option);

    return promise;
  }

  get(url, option) {
    this.doHttp(url, option, "get");
  }

  post(url, data, option) {
    this.doHttp(url, option, "post", data || "");
  }

  put(url, data, option) {
    this.doHttp(url, option, "put", data || "");
  }

  deleteMethod(url, data, option) {
    this.doHttp(url, option, "delete", data || "");
  }
